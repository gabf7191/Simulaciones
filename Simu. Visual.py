...
#Defino constantes (uso un for por que ni puta idea como juntarlo sino)
for i in range (1):
...
class Dot(pygame.sprite.Sprite):
    ...
    def __init__(
        ...
        self.Destino = np.random.choice(Salidas)
        self.Ramal = np.random.choice(Ramal)
...
class Cajon(pygame.sprite.Sprite):
    ...
    def relle(self,cc):
        ...
        elif cc==1 : self.image = Mycajon14   
                      
        elif cc==2 : self.image = Mycajonsemi  
                      
        elif cc==3 : self.image = Mycajon34
...
for i in range(T):
    ...
    if i%50 == 0:
        ...
        y = 200
        velx = -1
        guy = Dot(x, y, WIDTH, HEIGHT, color=Blue, velocity=velx)
        container.add(guy)
